...
<head>
  ...
  <title>Edit Laporan Keuangan - Biro Umum dan ASD DKI Jakarta</title>
...
<body class="bg-gray-100 text-black flex flex-col min-h-screen" style="background-image: url('{{ asset('img/batik.jpg') }}'); background-size: cover; background-position: center;">
  ...
  <div class="flex flex-col min-h-screen bg-white/95">
    ...
    <main class="container mx-auto px-6 py-10">
      <h1 class="text-4xl font-bold text-center text-[#0077b6] mb-10" data-aos="fade-down">Edit Laporan Keuangan</h1>

      <form action="{{ route('admin.laporan_keuangan.update', $laporan->id) }}" method="POST" enctype="multipart/form-data" class="bg-white p-6 rounded-xl shadow space-y-6 max-w-2xl mx-auto" data-aos="fade-up">
        @csrf

        <div>
          <label class="block text-gray-700 font-medium mb-1">Judul</label>
          <input type="text" name="judul" required value="{{ $laporan->judul }}"
        ...
        <div>
          ...
          <textarea name="deskripsi" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 bg-white focus:ring focus:ring-blue-300 focus:outline-none">{{ $laporan->deskripsi }}</textarea>
        </div>

        @if ($laporan->file_path)
          <p class="text-sm text-gray-600">
            File Saat Ini: 
            <a href="{{ asset('storage/' . $laporan->file_path) }}" target="_blank" class="text-blue-600 underline">
